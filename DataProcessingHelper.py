...
def filterEEG(eeg_data, fs=sampling_frequency, f_range=(1, 50)):
...
def bandpass_bandstop_filter(data,fs=sampling_frequency, lowcut=1, highcut=50, order = 2):
    nyq = 0.5 * fs
    low = lowcut / nyq
    high = highcut / nyq
...
def getEEGEpochs(epoch_times, eeg_df, eeg_chans, target_num_trials=750):
    ...
    for epoch_time in epoch_times: 
        ...
        print("Length of subdf", len(sub_df))
        print("time between interval", epoch_time[1] - epoch_time[0])
        if num_above >= 0:
            epoch = np.array(sub_df.values[num_above // 2: len(sub_df) - num_above // 2])[:target_num_trials]
            eeg_epochs.append(epoch.T)
            if len(epoch) != target_num_trials:
                print("Warning: Potential off by 1 error. Found trail with != 100 samples:", len(epoch))
        else:
...
# PSD plotting
def plotPSD(freq, psd, fs=sampling_frequency, pre_cut_off_freq=0, post_cut_off_freq=120, label=None):
...
def getFreqPSDFromEEG(eeg_data, fs=sampling_frequency):
    freq, psd = signal.periodogram(eeg_data, fs=int(fs), scaling='spectrum')
    return freq, psd

def getMeanFreqPSD(eeg_data, fs=sampling_frequency):
...
def plotPSD_fromEEG(eeg_data, fs=sampling_frequency, pre_cut_off_freq=0, post_cut_off_freq=120, label=None):
    freq, psd = signal.periodogram(eeg_data, fs=int(fs), scaling='spectrum')
    plotPSD(freq, psd, fs, pre_cut_off_freq, post_cut_off_freq, label)

# Get Powerbin information from EEG data
def getPowerRatio(eeg_data, binning, eeg_fs=sampling_frequency):
