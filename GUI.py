...
# Recording Settings 
LIVE_DATA = True ## Change this to true if running with live data
CHANNELS_USED = [0, 1, 2, 3] # Corresponding to the pins used on the Ganglion
FS = 200

# Calibration Trials
num_trials = 10  # Increase num_trials to 10 or more for live runs. Set to a low number if testing with random data
a1 = [1]*num_trials
a2 = [2]*num_trials
a3 = [3]*num_trials
a_123 = a1 + a2 + a3
...
LIVE_EEG_OUTPUT_FILENAME = "./data/eeg_data_10per3class.csv"
LIVE_EVENT_OUTPUT_FILENAME = "./data/event_data_10per3class.csv"
...
TRIAL_BASEEXTRA_TIME = TRIAL_FINISH_TIME + 3000 # 0.5s to finish trial
MAX_EXTRA_TIME = 2000 # randomly give an extra 0 to MAX_EXTRA_TRIAL time between trials
...
class DataViewer(tk.Frame): 
    ...
    def __init__(self, parent, controller, eeg_sampler, session = None):
        ...
        self.x_values = np.linspace(0, self.num_seconds_to_display, FS * self.num_seconds_to_display) # replace this in real test
...
class FeedbackPrompt(tk.Frame):
    ...
    def sendCommands(self):
        if self.can_update:
            ...
            self.label_trial.after(FS, self.sendCommands)
