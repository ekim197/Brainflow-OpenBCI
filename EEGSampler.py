...
SCALE_FACTOR_EEG = 1.2 * 8388607.0 * 1.5 * 51.0 # V/count
...
DEFAULT_FS = 200
DEFAULT_CHANNELS = [0, 1, 2, 3] # These are all the Ganglion channels
...
class EEGSampler:
        ...
        if not self.started:
            ...
            if self.live: 
               ...
               # self.eeg_thread = threading.Thread(target=self.board.start_stream, args=(self.push_data_sample,))   
                self.four_seconds_ago = datetime.now().seconds
                self.four_second_data = []
                self.eeg_thread = threading.Thread(target=self.board.start_stream, args=(callback))
    ...
    def __generate_artificial_data(self, callback) : 
        ...
        while self.started and (limit_counter < (600 * self.fs)): # TODO: remove 
            fake_sample = np.random.rand(4) * 300 # Ganglion has 4 channels by default, so we generate a value for each. 
    ...
    def callback(self, sample):
        ...
        if now - self.four_seconds_ago <= 0.005:
            self.four_seconds = now
    ...
    def preprocessing(self, data):
        processed_data =  np.array(data)[self.channels]
        processed_data_final = []
        for i in range(len(self.channels)):
            current_col = processed_data[:i]
            proccessed_data_final = np.concatenate(proccessed_data_final, current_col)

        processed_data_final = np.array(processed_data_final)[self.channels]
        print(processed_data_final)
        print("length of processed data =", len(processed_data_final[0,0]))

    '''
    Old callback function for reference
    def push_data_sample(self, sample):
        ...
        if (now - self.last_updated) > self.update_seconds :
            self.last_updated = nowelf.channels] * SCALE_FACTOR_EEG
        else :
